...
public:
     ...
     */
    template<typename T, typename Q = base_deduction< Supplier, T >>
    Environment& addSupplier(shared_ptr< T >);
    /**
     * Shorthand to addSupplier. You can't control the registered type
     * though which may cause troubles with multi-inherant suppliers
     */
    template<typename T>
    Environment& operator+=(shared_ptr< T >);
    /**
