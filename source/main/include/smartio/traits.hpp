...
template<template<typename> class B, typename Q, typename T, bool = true, bool = false>
...
_base_info <Base, Q, T > _base_deduction_impl(const Base< T > &);
template<template<typename > class Base, typename Q>
_base_info <Base, Q, void, false> _base_deduction_impl(...);
