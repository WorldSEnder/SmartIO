...
namespace io
{
  namespace defaultsuppliers
  {
    ...
    template<typename I, typename Mem>
      class stdconverter_t : public Supplier<I>, public Consumer<I>
      {
...
//	Default constructible is a needed for I, for dosupply()
	static_assert(std::is_default_constructible<I>::value,
	    "std type not default constructible?!?!?");bool
	doapply (ReadContext& reader, val_t& type) const override;
	void
	doconsume (WriteContext& reader, const I& type) const override;
      };
...
//    };
     ...
     */
    class netstringconverter : public Supplier<std::string>, public Consumer<
	std::string>
    {
      using typename Supplier<std::string>::item_t;
      ...
      doapply (ReadContext& reader, std::string& string) const override;
      void
      doconsume (WriteContext& writer, const std::string& string) const
	  override;
    };
