...
public:
     ...
     */
    template<typename T>
    BoundSupplier< T > getSupplier(supplier_key< T > key);
    /**
     ...
     */
    template<typename T>
    supply_t< T > get(supplier_key< T > key);
    template<typename T>
    T construct(supplier_key< T > key);
    /**
...
private:
    ...
    template<typename S>
    SupplierPtr< S > retrieveSupplier(supplier_key< S > key) const;
    // Finds the "default supplier", currently simple the first one
    // in the map...
    template<typename S>
    SupplierPtr< S > retrieveSupplier() const;
};
