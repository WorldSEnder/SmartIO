...
auto ReadContext::getStream()
-> input&
{
    return stream;
}

auto ReadContext::getStream() const
-> const input&
{
    return stream;
}

void ReadContext::setValue(key_t key, value_t val)
{
    contextVars[key] = val;
}

ReadContext::value_t ReadContext::getValue(key_t key, value_t defaultVal) const
{
...
bool ReadContext::hasValue(key_t key) const
{
